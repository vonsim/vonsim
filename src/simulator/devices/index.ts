  ...
  setIOMemory: (address, size, value) => {
    ...
    const byte = (address: number, value: number) =>
      match<number, SimulatorResult<void>>(address)
        ...
        .with(0x30, () => Ok(get().devices.pio.set("PA", value)))
        .with(0x31, () => Ok(get().devices.pio.set("PB", value)))
        .with(0x32, () => Ok(get().devices.pio.set("CA", value)))
        .with(0x33, () => Ok(get().devices.pio.set("CB", value)))
