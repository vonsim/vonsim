...
class EditorUI(s: VonSimState, defaultCode: String, onchange: () => Unit)
    extends VonSimUI(s) {
  ...
  val container = div(id := "aceEditor", editor.container).render
  ...
  def compilationEvent() {
    ...
    s.c match {
      case Left(f) => {
        ...
        val globalErrorAnnotations =
          f.globalErrors.map(e => Annotation(0, 0, e.msg, "global_error"))
...
//        println(f.instructions)
      ...
      case Right(f) => {
        val annotations = f.instructions.map(e => {
          Annotation(
            e.line.toDouble - 1,
            0.toDouble,
            s.uil.describeInstruction(e.instruction),
            "Correct Instruction"
          )
        })
        val warningAnnotations = f.warnings.map(w => {
          Annotation(w._1.toDouble - 1, 0.toDouble, w._2, "Warning")
        })

        session.setAnnotations((annotations ++ warningAnnotations).toJSArray)
        warningAnnotations.indices.foreach(
          i => session.addGutterDecoration(i.toDouble - 1, "ace_warning")
        )
  ...
  def instructionsToAnnotations(
    instructions: List[Either[CompilationError, InstructionInfo]]
  ) = {
    instructions.map(e => {
      e match {
        case Left(LexerError(l: Location, m: String)) =>
          Annotation(l.line.toDouble - 1, l.column.toDouble, m, "Lexer Error")
        case Left(ParserError(l: Location, m: String)) =>
          Annotation(l.line.toDouble - 1, l.column.toDouble, m, "Parser Error")
        case Left(x: SemanticError) =>
          Annotation(
            x.location.line.toDouble - 1,
            x.location.column.toDouble,
            x.msg,
            "Semantic Error"
          )
        case Right(x) =>
          Annotation(
            x.line.toDouble - 1,
            0.toDouble,
            s.uil.describeInstruction(x.instruction),
            "Correct Instruction"
          )
      }
    })
  ...
  def clearGutterDecorations(session: IEditSession) {
    (0 until session.getLength().toInt)
      .foreach(row => session.removeGutterDecoration(row, "ace_error"))
    (0 until session.getLength().toInt)
      .foreach(row => session.removeGutterDecoration(row, "ace_warning"))
  }
