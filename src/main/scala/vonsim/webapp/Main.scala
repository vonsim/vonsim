...
class Languages(
  ...
  val compilerLanguage: CompilerLanguage,
  val simulatorLanguage: SimulatorLanguage
) {}
...
object URLParametersParser {
  def apply(url: String) = {
    ...
    val parametersTuple = parameters
...
object Main extends JSApp {
  ...
  def getInitialCode(
    ...
    initializationFunction: Function1[String, Unit]
  ) {
    ...
    if (parameters.keySet.contains(codeURLKey)) {
      ...
      val promise =
        Ajax.get(url, timeout = 5000, headers = headers, responseType = "text")
      promise.onSuccess {
        case xhr =>
  ...
  def getLanguageCode(): String = {
    ...
    if (language == null) {
      if (dom.window.navigator.language != null && dom.window.navigator.language
            .trim() != "") {
  ...
  def languageCodeToObject[T](
    languageToObject: Map[String, T],
    language: String
  ) = {
  ...
  def getLanguages(language: String) = {
    ...
    val compilerLanguage =
      languageCodeToObject(CompilerLanguage.codes, language)
    val simulatorLanguage =
      languageCodeToObject(SimulatorLanguage.codes, language)
