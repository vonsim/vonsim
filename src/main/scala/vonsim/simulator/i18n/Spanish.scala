...
class Spanish extends SimulatorLanguage {
  ...
  def invalidMemoryAddress(address: Int) =
    s"La dirección de memoria $address es inválida, es decir, está fuera del rango permitido."
  def instructionNotImplemented(instruction: String) =
    s"La instrucción $instruction no ha sido implementada"

  def instructionErrorMessage(i: InstructionInfo) =
    s"Error ejecutando instrucción ${i.rawInstruction}."
  def reason = "Razón"
