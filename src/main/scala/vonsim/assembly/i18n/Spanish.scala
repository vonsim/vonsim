...
class Spanish extends CompilerLanguage {
  ...
  def emptyProgram =
    "El programa no contiene instrucciones. Debe tener, como mínimo, una sentencia END."
  ...
  def onlyOneEnd =
    "Debería haber una sola sentencia END, y debe ser la última del programa."
  def loopsInEqu =
  ...
  def labelUndefined(label: String) =
    s"La etiqueta ${label} no ha sido definida."
  def labelsUndefined(labels: List[String]) =
    if (labels.length == 1) labelUndefined(labels(0))
    else s"Las etiquetas (${labels.mkString(", ")}) no están definidas."
  def operandNotUpdatable(operand: String) =
    s"El operando $operand no puede modificarse, es un valor."
  def immediateOperandsNotUpdatable(operand: String) =
    s"El operando $operand es inmediato, no puede modificarse."
  def dontFitIn16Bits(v: Int) =
    s"El número ${v} no puede ser representado con 8 o 16 bits"
  def dontFitIn16Bits =
  ...
  def literalStringsAsImmediate(s: String) =
    s"No se puede usar strings literales ($s) como operandos inmediatos."
  def cannotDetermineMemoryReferenceType =
    s"No se puede determinar el tipo de referencia a memoria (word o byte)."
  def memoryMemory =
    "Ambos operandos acceden a la memoria. No se puede acceder a la memoria con dos operandos distintos en una misma instrucción."
  def DWordWordOperands =
    "El segundo operando requiere solo 8 bits para representarsepero el primer operando tiene 16 bits. Entonces, no está claro si se debe escribir el resultado sobre los primeros o últimos 8 bits."
  def indirectPointerTypeUndefined =
    "El direccionamiento indirecto ([bx]) con un operando inmediato requiere que se especifique el tipo de puntero con las palabras clave WORD PTR (para operandos inmediatos de 16 bits) o BYTE PTR (para operandos de 8 bits). Por ejemplo, MOV WORD PTR [BX], 15."
  def WordDWordOperands =
    "El segundo operando requiere 16 bits para representarse, pero el primero sólo tiene 8 bits"
