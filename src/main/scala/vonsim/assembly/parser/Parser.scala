...
object Parser extends MyParsers {
  ...
  def apply(tokens: Seq[Token]): Either[ParserError, Instruction] = {
    ...
    program(reader) match {
      ...
      case NoSuccess(msg, next) =>
        Left(
          ParserError(Location(next.pos.line, next.pos.column), defaultMessage)
        )
  ...
  def program = positioned {
    (labeledInstruction | instruction) ~ newline ^^ {
      case (o: Instruction) ~ _ => o
    }
  }
  ...
  def equ = positioned {
    (identifier ~ EQU() ~ expression) ^^ {
      case ((i: IDENTIFIER) ~ (o: EQU) ~ (e: Expression)) => EQUDef(i.str, e)
    }
  }
  ...
  def binaryArithmetic: Parser[Instruction] = positioned {
    (binary ~ operand ~ COMMA() ~ operand) ^^ {
      case ((o: BinaryArithmeticOp) ~ (m: Operand) ~ _ ~ (v: Operand)) =>
        BinaryArithmetic(o, m, v)
    }
  }
  ...
  def unaryArithmetic: Parser[Instruction] = positioned {
    (unary ~ operand) ^^ {
      case ((o: UnaryArithmeticOp) ~ (m: Operand)) => UnaryArithmetic(o, m)
    }
  }
  ...
  def stack = positioned {
    ((PUSH() | POP()) ~ fullRegister) ^^ {
      case (o: StackInstruction) ~ (t: FullRegisterToken) => Stack(o, t)
    }
  }
  ...
  def jmp = positioned {
    (JMP() ~ identifier) ^^ {
      case JMP() ~ IDENTIFIER(i) => UnconditionalJump(i)
    }
  }
  ...
  def vardef = positioned {
    ...
    val ints = ((identifier ?) ~ (DB() | DW()) ~ varDefInts) ^^ {
      case Some(IDENTIFIER(id)) ~ (t: VarType) ~ (e: List[
            Either[Undefined.type, Expression]
          ]) =>
        VarDef(id, t, e)
      case None ~ (t: VarType) ~ (e: List[
            Either[Undefined.type, Expression]
          ]) =>
        VarDef(Random.alphanumeric.take(30).mkString, t, e)
    }
    val str = ((identifier ?) ~ DB() ~ literalString) ^^ {
      case Some(IDENTIFIER(id)) ~ DB() ~ LITERALSTRING(s) =>
        VarDef(id, DB(), stringToIntList(s))
      case None ~ DB() ~ LITERALSTRING(s) =>
        VarDef(Random.alphanumeric.take(30).mkString, DB(), stringToIntList(s))
    }
    str | ints
  }

  def stringToIntList(s: String) =
    s.map((c: Char) => Right(ConstantExpression(c.charValue().toInt))).toList

  //
  def varDefValue =
    (expression ^^ { case e => Right(e) } | UNINITIALIZED() ^^ {
      case u                => Left(Undefined)
    })

  def varDefInts = rep1sep(varDefValue, COMMA())

  def conditionalJump = positioned {
    (conditionalJumpTokens ~ identifier) ^^ {
      case (o: ConditionalJumpToken) ~ IDENTIFIER(i) => ConditionalJump(o, i)
    }
  }
  ...
  private def literalString = positioned {
    accept(compilerLanguage.stringLiteral, {
      case lit @ LITERALSTRING(v) => lit
    })
  }
  ...
  private def literalInteger = positioned {
    accept(compilerLanguage.integerLiteral, {
      case lit @ LITERALINTEGER(v) => lit
    })
  }
  ...
  private def indirect = {
    val indirectDWord = (WORD() ~ PTR() ~ INDIRECTBX()) ^^ {
      case (WORD() ~ PTR() ~ INDIRECTBX()) => DWORDINDIRECTBX()
    }
    val indirectWord = (BYTE() ~ PTR() ~ INDIRECTBX()) ^^ {
      case (BYTE() ~ PTR() ~ INDIRECTBX()) => WORDINDIRECTBX()
    }
  ...
  private def allRegisters =
    (Token.registers map tokenAsParser) reduceLeft (_ | _)
  ...
  def offsetLabel =
    accept(compilerLanguage.offsetLabel, {
      case lit @ OFFSETLABEL(v) => copyPosition(OffsetLabelExpression(v), lit)
    })
  def negativeInteger = MinusOp() ~ integer ^^ {
    case m ~ i => copyPosition(ConstantExpression(-i.v), i)
  }

  def integer =
    accept(compilerLanguage.integerLiteral, {
      case lit @ LITERALINTEGER(v) => copyPosition(ConstantExpression(v), lit)
    })
  def expressionLabel =
    accept(compilerLanguage.equLabel, {
      case lit @ IDENTIFIER(v) => copyPosition(LabelExpression(v), lit)
    })

  def parens: Parser[Expression] = OpenParen() ~> expression <~ CloseParen()
  def term =
    (negativeInteger | integer | offsetLabel | expressionLabel | parens)

  def binaryOp(level: Int): Parser[((Expression, Expression) => Expression)] = {
    level match {
      case 1 =>
        PlusOp() ^^^ { (a: Expression, b: Expression) =>
          BinaryExpression(PlusOp(), a, b)
        } |
          MinusOp() ^^^ { (a: Expression, b: Expression) =>
            BinaryExpression(MinusOp(), a, b)
          }
      case 2 =>
        MultOp() ^^^ { (a: Expression, b: Expression) =>
          BinaryExpression(MultOp(), a, b)
        } |
          DivOp() ^^^ { (a: Expression, b: Expression) =>
            BinaryExpression(DivOp(), a, b)
          }
