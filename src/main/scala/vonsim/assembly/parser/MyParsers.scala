...
trait MyParsers {
  ...
  private lazy val lastNoSuccessVar =
    new DynamicVariable[Option[Option[NoSuccess]]](None)

  /** A common super-class for unsuccessful parse results. */
  sealed abstract class NoSuccess(val msg: String, override val next: Input)
      extends ParseResult[Nothing] { // when we don't care about the difference between Failure and Error
    ...
    def mapPartial[U](
      f: PartialFunction[Nothing, U],
      error: Nothing => String
    ): ParseResult[U] = this

    def flatMapWithNext[U](
      f: Nothing => Input => ParseResult[U]
    ): ParseResult[U] = this
  ...
  abstract class Parser[+T] extends (Input => ParseResult[T]) {
    ...
    def withFilter(p: T => Boolean): Parser[T] = Parser { in =>
      this(in) filterWithError (p, "Input doesn't match filter: " + _, in)
    }
    ...
    /** A parser combinator for non-back-tracking sequential composition.
      ...
      */
    def ~![U](p: => Parser[U]): Parser[~[T, U]] = OnceParser {
  ...
  implicit def accept(e: Elem): Parser[Elem] =
    acceptIf(_ == e)("Esperaba '" + e + "' pero encontre " + _ + ".")
  ...
  /** A parser matching input elements that satisfy a given predicate.
    ...
    */
  def acceptIf(p: Elem => Boolean)(err: Elem => String): Parser[Elem] = Parser {
    in =>
      if (in.atEnd) Failure("fin del código", in)
  ...
  def acceptMatch[U](expected: String, f: PartialFunction[Elem, U]): Parser[U] =
    Parser { in =>
      ...
      else Failure(expected + " esperado", in)
    }
  ...
  /** A parser that always fails.
    ...
    */
  def failure(msg: String) = Parser { in =>
    Failure(msg, in)
  }
  ...
  /** A parser generator for non-empty repetitions.
    ...
    */
  def rep1[T](first: => Parser[T], p0: => Parser[T]): Parser[List[T]] = Parser {
    in =>
      lazy val p = p0 // lazy argument
  ...
  def not[T](p: => Parser[T]): Parser[Unit] = Parser { in =>
    p(in) match {
      case Success(_, _) => Failure("Falla esperada", in)
      case _             => Success((), in)
    }
  }
  ...
  def positioned[T <: Positional](p: => Parser[T]): Parser[T] = Parser { in =>
    p(in) match {
      case Success(t, in1) =>
        Success(if (t.pos == NoPosition) t setPos in.pos else t, in1)
      case ns: NoSuccess => ns
    }
  }
      ...
      p(in) match {
        case s @ Success(out, in1) =>
          if (in1.atEnd)
            ...
            lastNoSuccessVar.value flatMap (_ filterNot { _.next.pos < in1.pos }) getOrElse Failure(
  ...
  trait OnceParser[+T] extends Parser[T] {
    override def ~[U](p: => Parser[U]): Parser[~[T, U]] = OnceParser {
      (for (a <- this; b <- commit(p)) yield new ~(a, b)).named("~")
    }
  }
