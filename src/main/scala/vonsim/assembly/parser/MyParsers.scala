...
trait MyParsers {
  ...
  case class Failure(override val msg: String, override val next: Input) extends NoSuccess(msg, next) {
    ...
    override def toString = "["+next.pos+"] fallo: "+msg+"\n\n"+next.pos.longString
   ...
   *  @return
   */
  def elem(kind: String, p: Elem => Boolean) = acceptIf(p)(inEl => kind+" esperado")
  ...
  implicit def accept(e: Elem): Parser[Elem] = acceptIf(_ == e)("Esperaba '"+e+"' pero encontre " + _ + ".")
