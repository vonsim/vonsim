...
case class LabeledInstruction(label: String, i: ExecutableInstruction)
    extends ExecutableInstruction
    with LabelDefinition
...
case class VarDef(
  label: String,
  t: VarType,
  values: List[Either[Undefined.type, Expression]]
) extends Instruction
    with LabelDefinition
case class EQUDef(label: String, expression: Expression)
    extends Instruction
    with NonAddressableInstruction
    with LabelDefinition
