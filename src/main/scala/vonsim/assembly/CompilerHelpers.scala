...
abstract class InstructionSemanticError(i: parser.Instruction)
    extends SemanticError {
  def location = i.location
}
case class MemoryMemoryReferenceError(i: parser.Instruction)
    extends InstructionSemanticError(i) {
  def msg = Compiler.language.memoryMemory
}

case class IndirectPointerTypeUndefined(i: parser.Instruction)
    extends InstructionSemanticError(i) {
  def msg = Compiler.language.indirectPointerTypeUndefined
}
case class WordDWordOperandSizeMismatchError(i: parser.Instruction)
    extends InstructionSemanticError(i) {
  def msg = Compiler.language.WordDWordOperands
}
case class DWordWordOperandSizeMismatchError(i: parser.Instruction)
    extends InstructionSemanticError(i) {
  def msg = Compiler.language.DWordWordOperands
}

case class GenericSemanticError(p: Positional, msg: String)
    extends SemanticError {
...
abstract class Resolver {
  ...
  def getMemoryLabelFromMemoryExpression(e: parser.Expression): Option[String] =
    e match {
      case BinaryExpression(op, a, b) =>
        getMemoryLabelFromMemoryExpression(a) match {
  ...
  def memoryExpressionType(e: parser.Expression) =
    getMemoryLabelFromMemoryExpression(e) match {
      case None    => None
  ...
  def undefinedLabels(e: parser.Expression): List[String] = {
    e match {
      ...
      case LabelExpression(l) =>
        if (equOrVardefLabelDefined(l)) List() else List(l)
      case OffsetLabelExpression(l) =>
        if (vardefLabelDefined(l)) List() else List(l)
    }
  }
...
//  val jumpLabels:List[String]
  val equ = instructions
    .collect({ case Right(x: parser.EQUDef) => (x.label, x.expression) })
    .toMap
  val (vardefLabelToLine, vardefLabelToType, jumpLabelToLine) =
    getLabelToLineMappings(instructions)
  ...
  def vardefLabelDefined(label: String) =
    vardefLabelToLine.keySet.contains(label)
  ...
  def getLabelToLineMappings(instructions: ParsingResult): (
    Map[String, Compiler.Line],
    Map[String, VarType],
    Map[String, Compiler.Line]
  ) = {
    ...
    val unlabeledInstructions = instructions.rights.foreach(_ match {
...
class SecondPassResolver(
  val instructions: List[InstructionInfo],
  val firstPassResolver: FirstPassResolver
) extends Resolver {

  def vardefLabelToType = firstPassResolver.vardefLabelToType
  def equ = firstPassResolver.equ

  val (vardefLineToAddress, executableLineToAddress) = getMemoryLayout(
    instructions
  )
  val vardefLabelToAddress = firstPassResolver.vardefLabelToLine.map(
    x => (x._1, vardefLineToAddress(x._2))
  )
  val jumpLabelToAddress = firstPassResolver.jumpLabelToLine.map(
    x => (x._1, executableLineToAddress(x._2))
  )
  ...
  def vardefLabelDefined(label: String) =
    vardefLabelToAddress.keySet.contains(label)
  def jumpLabelDefined(label: String) =
    jumpLabelToAddress.keySet.contains(label)
  def equLabelDefined(label: String) = equ.keySet.contains(label)
  ...
  def getMemoryLayout(instructions: List[InstructionInfo]) = {
    ...
    val vardefLineToAddress =
      mutable.Map[Compiler.Line, Compiler.MemoryAddress]()
    val executableLineToAddress =
      mutable.Map[Compiler.Line, Compiler.MemoryAddress]()
