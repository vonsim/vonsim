...
export function evaluateInstruction(
  ...
  return match<ValidatedInstructionStatement, ProgramInstruction>(statement)
    ...
    .with({ type: jumpInstructionPattern }, statement => {
      ...
      if (!label) {
        throw new CompilerError("label-not-found", statement.jumpTo).at(statement);
      }
      if (label.type !== "instruction") {
        throw new CompilerError("label-should-be-an-instruction", statement.jumpTo).at(statement);
      }
    ...
    .with({ type: intInstructionPattern }, statement => {
      ...
      if (!isMatching(interruptPattern, int)) {
        throw new CompilerError("invalid-interrupt", int).at(statement.interrupt);
      }
...
function evaluateAddress(
  ...
  if (computed < MIN_MEMORY_ADDRESS || computed > MAX_MEMORY_ADDRESS) {
    throw new CompilerError("address-out-of-range", computed).at(address);
  }
  if (codeMemory.has(computed)) {
    throw new CompilerError("address-has-code", computed).at(address);
  }
  return computed;
}
