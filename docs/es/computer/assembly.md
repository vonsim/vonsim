...
Los registros accesibles por el usuario de 16 bits son `AX`, `BX`, `CX`, `DX`, `SP` y `FP`. Los registros accesibles por el usuario de 8 bits son `AL`, `AH`, `BL`, `BH`, `CL`, `CH`, `DL` y `DH`. Al utilizar cualquiera de estos registros, el ensamblador puede inferir automáticamente si se tratará de una operación de 8 o 16 bits.
...
Los valores inmediatos son valores que se pueden determinar al momento de ensamblar el programa. Por ejemplo, en `mov ax, 5` se sabe que se debe copiar el número `5` en `AX`, por lo que el número `5` es un valor inmediato que se almacena en memoria. En cambio, en `mov ax, [fp]` no se sabe qué valor se debe copiar en `AX`, ya que depende del contenido de `FP`. Por lo tanto, `[fp]` no es un valor inmediato.
